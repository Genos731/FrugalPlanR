...
public class AddBudgetServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Get parameters
		Double goalValue = new Double(request.getParameter("amount"));	
		Calendar calendarDate = new GregorianCalendar();
		calendarDate.setTimeInMillis(Long.parseLong(request.getParameter("date")));
		Date date = (Date) calendarDate.getTime();
		...
		// Create budget
		try {
			accessor.create(account, goalValue, date, date);
